...
description: Step-by-step hardware installation guide for GaggiMate on Gaggia Classic machines.
...
Prepare all parts, materials and tools listed in the sourcing pages. Follow these guides if you haven't already: [Printed Parts](/docs/sourcing-printed-parts) and [Flashing](/docs/flashing).
...
[Classic Wiring Schematics](/downloads/Classic-Wiring.pdf) PDFs.
...
Pry the rectangular clamp around the switches open and move it a little towards the inside of the machine. We will remove it later.
...
Find the flat metal spring on both sides of the switches section that hold it in place.
...
Compare your stock wiring with the [Classic Wiring Schematics](/downloads/Classic-Wiring.pdf). In case something, i.e. your wire coloring, differs from the PDF, you should be able to understand and adapt the guide to your individual machine.
...
We recommend making more room in order to more easily access the parts and wires:
...
- Remove the steam knob by pulling it firmly. Make sure you do not lose the metal spring inside the knob.
...
Optional: This gives you more space for the installation.
...
Find the steam thermostat on TOP of the boiler. Unplug the wires ST.A and ST.B.
...
To keep the switches operational, plug the <u>Buttons cable</u> into the switches section as follows:
Green  BREW to C.1
Green STEAM to S.1
Black Gnd to C.2
Black Gnd to S.2
...
Connect the <u>SSR signal cable</u> to the lower section of the SSR.
...
<div className="flex flex-row gap-4">
  ...
  <Preview src={img_64} width={400} alt="" />
...
Make sure the screw hole is free of old thermal paste and gunk.
...
If you have thermal paste available, apply some to the threads and tip of the thermo sensor.
...
Measure the wire ends of the pressure sensor. They should have an un-insulated length of 11mm.
If needed, strip the insulation accordingly.
...
Assemble the pressure sensor, silicone gasket and quick-connect nut.
If needed, strip the insulation accordingly.
...
Make sure the gasket is plain, not squeezed and the center hole is free.
...
Take the <u>3-way T-connector</u> and find a good spot between the pump outlet and the 3-way solenoid to install the T-connector.
...
Prefill with water. Either use a syringe or submerge.
Make sure there are no bubbles or air in the sensor.
...
The screen should turn on and show the Standby screen of GaggiMate. Touch the screen once to switch to warmup mode. You should see the current room temperature being displayed and the LED on the SSR will light up, indicating it's heating up. Check that nothing is leaking inside the machine.
...
You have now finished setting up your Gaggia Classic with GaggiMate and can start making coffee.
